...
- Retried event deliveries (`x-slack-retry-num`, socket `retry_num`) are dropped when the original delivery was already dispatched, using an `event_id` marker in the state adapter (1-hour TTL). Events whose first delivery never arrived are still recovered via the retry.
