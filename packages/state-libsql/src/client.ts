...
export interface LibSqlStateAdapterOptions {
  ...
  authToken?: string;
  /** Additional `@libsql/client` options (encryption, sync, tls, intMode, …). */
...
export interface LibSqlStateClientOptions {
  /** Existing `@libsql/client` Client instance. */
  client: Client;
  /** Key prefix for all rows (default: "chat-sdk") */
  keyPrefix?: string;
  /** Logger instance for error reporting */
...
/**
 * libSQL state adapter backed by `@libsql/client`.
 *
 * Use this when you want a pure-JS driver that tree-shakes cleanly in edge /
 * serverless environments (Vercel Edge, Cloudflare Workers). For local Node
 * with the native binding, import from `@chat-adapter/state-libsql` instead.
 */
    ...
    try {
      ...
      await tx.execute({
        sql: `INSERT INTO chat_state_queues (key_prefix, thread_id, value, expires_at)
              VALUES (?, ?, ?, ?)`,
        args: [this.keyPrefix, threadId, serialized, entry.expiresAt],
      });
