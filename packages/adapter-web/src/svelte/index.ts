 ...
 *
 * Wraps `@ai-sdk/svelte`'s `Chat` class with a `DefaultChatTransport` pointed
 * at `/api/chat` (or whatever you pass as `api`). The returned instance has
 * Svelte-reactive `messages`, `status`, and `error` properties — access them
 ...
 * use `@ai-sdk/svelte`'s `Chat` class directly.
 */
...
export type { Chat } from "@ai-sdk/svelte";
