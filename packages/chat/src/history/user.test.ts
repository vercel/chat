...
const USER_KEY_REQUIRED_RE = /history\.user\.append.*userKey is required/;
...
describe("UserHistoryApiImpl", () => {
  let state: MockStateAdapter;
  let api: UserHistoryApiImpl;

  beforeEach(() => {
    state = createMockState();
    api = new UserHistoryApiImpl(state, {});
  });
  ...
  describe("append", () => {
    ...
    it("includes formatted when storeFormatted is true", async () => {
      api = new UserHistoryApiImpl(state, { storeFormatted: true });
      const thread = createTestThread();
    ...
    it("passes retention duration string through as ttlMs", async () => {
      api = new UserHistoryApiImpl(state, { retention: "7d" });
      const thread = createTestThread();
      const msg = createTestMessage("m1", "Hello");
      msg.userKey = "u1";
