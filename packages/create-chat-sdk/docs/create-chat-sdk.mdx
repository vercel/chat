...
The interactive prompt lists official adapters by default. Pass `--all` to also include vendor-official adapters, grouped under **Official** and **Vendor-official** headings. Either way, you can install any supported adapter directly with `--adapter`.

Adapters that require a long-running process — Matrix and Lark — cannot run on the webhook-only serverless runtime and are not available through the CLI; add them to an existing project manually instead.
...
| `--all` | List every adapter (official and vendor-official) in the interactive prompt. |
