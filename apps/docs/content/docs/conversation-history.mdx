...
<Callout type="warn">
  This page covers the older `bot.transcripts` API. The new **[History API](/docs/history)** (`bot.history.user`) is a drop-in replacement with an expanded feature set — user, thread, and channel scopes in one unified namespace. Migrate by renaming your config key from `transcripts` to `history.user` and updating call sites from `bot.transcripts.*` to `bot.history.user.*`.
</Callout>
...
`bot.history.user` (previously `bot.transcripts`) keeps a per-user transcript in your state adapter, keyed by a stable identifier you choose (an email, an internal user ID, anything that's the same person no matter where they are).

## Setup

You opt in by setting two fields on `ChatConfig`. The examples below use the new `history.user` key — `transcripts` still works as a deprecated alias:
...
const bot = new Chat({
  ...
  // identity is now inside history.user (was a top-level field with transcripts).
  history: {
    user: {
...
`history.user` requires an identity resolver — set `history.user.identity`, or keep the deprecated top-level `identity` field during migration. Omitting both when `history.user` is set throws at construction.
...
`identity` lives inside `history.user` and runs once per inbound message during dispatch. The `author`, `message`, and `adapter` name are all available:
...
await bot.history.user.list({ userKey: "mike@acme.com", limit: 50 });
...
See [History API reference](/docs/api/history) for full type signatures, configuration options, and the entry shape.
