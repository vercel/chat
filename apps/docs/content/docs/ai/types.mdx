...
Per-tool overrides accepted by `createChatTools({ overrides })`. `execute`, `inputSchema`, and `outputSchema` are intentionally excluded so tool semantics stay stable across upgrades.
