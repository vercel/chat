...
bot.onSlashCommand("/remind-cancel", async (event) => {
  ...
  const canceled = await cancelScheduledPost(scheduleId);
...
The cancellation path is intentionally simple. It only updates the database row. If the workflow wakes up later, it re-checks the row and exits without posting.
