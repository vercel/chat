...
## Define a hook for follow-up turns

Workflow hooks are how follow-up messages get injected back into a running session. Define the payload once so both the workflow and the webhook side use the same type:

```typescript title="workflows/chat-turn-hook.ts" lineNumbers
import type { SerializedMessage } from "chat";
import { defineHook } from "workflow";

export const chatTurnHook = defineHook<{
...
import { chatTurnHook } from "@/workflows/chat-turn-hook";
...
async function processMessage(
  ...
  await thread.post(await runTurn(text));
  return true;
}
...
export async function durableChatSession(payload: string) {
  ...
  using hook = chatTurnHook.create({ token: workflowRunId });
...
- `chatTurnHook.create({ token: workflowRunId })` makes the workflow run itself the session identifier
- `runTurn()` is a step, so it has full Node.js access for AI calls, databases, and HTTP clients
...
async function routeTurn(
  ...
  await chatTurnHook.resume(state.runId, {
    message: message.toJSON(),
  });
}
...
<Callout type="info">
  In production, catch `chatTurnHook.resume()` failures, clear stale `runId` values, and start a new session if the old workflow has already ended.
</Callout>
...
4. Follow-up messages call `chatTurnHook.resume(runId, ...)` instead of starting a new run.
