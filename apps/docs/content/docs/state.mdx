...
|---------|---------|----------|
| [Redis](/adapters/redis) | `@chat-adapter/state-redis` | Production (recommended) |
| [ioredis](/adapters/ioredis) | `@chat-adapter/state-ioredis` | Production (Cluster/Sentinel) |
| [PostgreSQL](/adapters/postgres) | `@chat-adapter/state-pg` | Production (SQL-first stack) |
| [Memory](/adapters/memory) | `@chat-adapter/state-memory` | Development and testing |

Each adapter page includes a full feature matrix covering persistence, multi-instance support, and platform-specific capabilities.
