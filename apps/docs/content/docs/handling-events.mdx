...
Currently, only the Teams adapter emits them, including personal installs.

```typescript
bot.onInstalled(async (event) => {
  if (event.adapter.name !== "teams" || event.action !== "add") return;
  // Persist installation metadata or start your onboarding flow.
  await recordInstallation(event.tenantId, event.conversationId);
});

bot.onUninstalled(async (event) => {
  if (event.adapter.name !== "teams" || event.action !== "remove") return;
  await removeInstallation(event.tenantId, event.conversationId);
});
```

Events include `adapter`, activity `id`, original `action`, platform
`conversationId`, optional actor `userId`, `tenantId`, `locale`, and `raw`.
The optional `channelId` is a normalized Chat destination. The optional
`conversationReference` is platform-specific serializable metadata; narrow it
before use. Teams omits both destination fields if no service URL was supplied.

Teams upgrade actions (`add-upgrade` and `remove-upgrade`) reach the corresponding
handler; check `action` before sending welcomes or deleting durable records.
A team install can also emit `onMemberJoinedChannel`, so keep each welcome flow
in one handler. Make side effects idempotent for webhook retries. Handler errors
are logged, and asynchronous work is registered with the webhook's `waitUntil`.

See [Teams installation lifecycle](/adapters/official/teams#installation-lifecycle)
for reference persistence, reinstall cleanup, and proactive messages.
