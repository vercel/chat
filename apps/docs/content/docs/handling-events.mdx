...
bot.onInstalled(async (event) => {
  ...
  await recordInstallation(event.tenantId, event.conversationId, event.channelId);
});
...
`conversationId`, and the optional actor `userId`, `tenantId`, `locale`, and
`raw`. The optional `channelId` is the normalized Chat destination; persist it
and post later with `bot.channel(channelId).post()`.

`action` is `add`, `add-upgrade`, `remove`, or `remove-upgrade`. Upgrades reach
the same handler as installs and removals, so check `action` before sending a
welcome or deleting durable records. A Teams team or group chat install also
