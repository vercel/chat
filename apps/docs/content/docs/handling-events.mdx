...
`onReaction` fires when users add or remove emoji reactions to messages. You can handle all reactions or filter by specific emoji. The [`emoji` helper](/docs/emoji) provides type-safe, cross-platform emoji names for filtering and reacting.
