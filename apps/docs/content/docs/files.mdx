...
On WhatsApp, each file or attachment is sent as its own message (one media object per API call). Markdown text becomes a caption on the first media message when supported; audio messages and captions over 1024 characters send the text as a separate message first. URL-only attachments must use HTTPS.
