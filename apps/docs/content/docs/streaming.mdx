...
### Consuming a stream without creating a message

A custom adapter can fully consume a `StreamingPlan` without creating a platform message. This is useful when every structured chunk is intentionally ignored or handled out of band. Return `STREAM_HANDLED_NO_MESSAGE` to tell Chat SDK that the stream is complete and core fallback must not run:

```typescript
import { STREAM_HANDLED_NO_MESSAGE } from "chat";

async function stream(threadId, chunks, options) {
  for await (const chunk of chunks) {
    // Handle supported chunks and intentionally ignore the rest.
  }
...
This result is valid only for streams wrapped in `StreamingPlan`, whose `thread.post()` result is the plan itself. A bare async iterable must create and return a message. Return `null` only before consuming any chunks when delegating the untouched stream to Chat SDK's post+edit fallback.
