...
By default, any user who can see the card and click a button decides the approval. The click is authenticated (Chat SDK verifies the platform signature, and the button's callback URL is never exposed to end users), so the `user.id` on the result is trustworthy.

To restrict *who* may decide, pass `approvers` with the allowed user IDs. Clicks from anyone else post a notice and the workflow keeps waiting:
...
Set `approvers` for anything consequential (deploys, spend, destructive actions). Without it, the result tells you who clicked, not whether they were allowed to.
