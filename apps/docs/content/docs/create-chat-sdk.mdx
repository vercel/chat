---
title: create-chat-sdk
description: Scaffold a new Chat SDK bot project with a single command.
---

`create-chat-sdk` is the fastest way to start a new Chat SDK project. It scaffolds a Next.js app with your chosen platform adapters, state adapter, environment variables, and webhook route - ready to run.
...
Every prompt can be skipped with flags, making `create-chat-sdk` fully scriptable for CI, automation, and AI agents:

```bash
npm create chat-sdk@latest my-bot -d 'My bot' --adapter slack teams redis -yq
```
