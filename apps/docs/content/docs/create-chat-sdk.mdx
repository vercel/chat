...
The CLI accepts any official or vendor-official adapter that runs on the generated webhook-only serverless runtime. Adapters that require a long-running process — Matrix and Lark — are not available through the CLI; add them to an existing project manually instead.
