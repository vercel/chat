...
Modals open form dialogs in response to button clicks. They support text inputs, dropdowns, radio buttons, and server-side validation. Currently supported on Slack.

## Open a modal

Modals are opened from [action handlers](/docs/actions) using `event.openModal()`:
