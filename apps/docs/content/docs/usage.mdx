...
For typed access to the platform's native API client, use the SDK-named getter (`.linearClient` for Linear, `.octokit` for GitHub):
...
The previous `.client` getter still works as a deprecated alias on both adapters.
