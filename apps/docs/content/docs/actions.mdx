...
| `values` | `Record<string, string>` (optional) | The card's own input values, keyed by input id |
...
## Read a text box on the card

On Teams a card can carry a [`TextInput`](/docs/cards), and what the user typed arrives on the action as `event.values`, keyed by input id — the same shape as `ModalSubmitEvent.values`:

```tsx title="lib/bot.tsx" lineNumbers
<Card title="Deploy to production?">
  <TextInput id="notes" label="Notes" multiline optional />
  <Actions>
...
```typescript title="lib/bot.ts" lineNumbers
bot.onAction("approve", async (event) => {
  const notes = event.values?.notes?.trim();
  // Typed by the user, so post it as a code span rather than raw markdown.
  await event.thread?.post(notes ? `Approved — notes: \`${notes}\`` : "Approved");
});
```

`values` is absent on platforms that cannot put an input on a card, and on cards that have none.

`event.values` is user input, not a trusted payload. Teams enforces `optional` and `maxLength` in its own UI, but a handler still has to validate what arrives — a value can be missing, empty, or longer than the limit. Echoing it back into a message unescaped lets a user put markdown, such as a link, into a message the bot appears to have written.
...
If the card carried inputs, what the user typed is included as `"values"`, keyed by input id — the same object the handler reads as `event.values`. It is omitted when the card has no inputs:
...
```json
{
  ...
  "user": { "id": "U123", "name": "alice" },
  "threadId": "teams:19%3Aabc:1700000000",
  "messageId": "1700000000"
}
```
