...
| Notion | `@chat-adapter/notion` | Yes* | No | Fallback | No | Post+Edit | No |
...
\* Notion mentions depend on `mentionMode` (`"mention"` | `"all-comments"` | `"keyword"`). Default `"mention"` matches plain-text `@userName` / `@botUserId` (Notion connections are not @-mentionable in the comment composer).
...
| `@chat-adapter/notion` | Notion comments adapter |
