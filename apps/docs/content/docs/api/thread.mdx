...
If you pass an adapter directly to `ThreadImpl.fromJSON(data, adapter)`, streaming can work without a registered Chat instance, using the default settings. Runtime settings and state are only resolved from a Chat instance that owns that exact adapter. State access throws if the registered instance owns a different adapter, rather than reading or writing another bot's state. Prefer `bot.reviver()` to bind ownership explicitly.
