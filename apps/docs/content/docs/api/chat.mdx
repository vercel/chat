...
<TypeTable
  type={{
    ...
    'event.values': {
      description: "The card's own input values, keyed by input ID. Absent where the platform has no card inputs.",
...
Fires when the bot is installed, including when an app upgrade adds the bot to the manifest. Routine upgrades do not emit this event. Currently emitted only by the Teams adapter, for personal, group chat, and team installs.
...
records with `event.raw.channelData.team.id`, not just `conversationId`, which
