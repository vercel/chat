...
Look up user information by user ID. Returns a `UserInfo` object with name, email, avatar, and bot status. Supported on Slack, Discord, Google Chat, GitHub, Linear, and Telegram. Other official adapters will throw `NOT_SUPPORTED`.
...
```typescript
// Or with an Author object from an event
const author = await bot.getUser(event.user);
```
...
Adapters that don't support user lookups will throw a `ChatError` with code `NOT_SUPPORTED`. Wrap calls in try-catch if your bot runs on multiple platforms:
