...
Access the platform's typed native API client directly via an SDK-named getter — `.linearClient` on Linear, `.octokit` on GitHub:
...
<Callout type="info">
  The previous `.client` getter still works on both adapters as a deprecated alias for `.linearClient` / `.octokit`.
</Callout>

<Callout type="warn">
  For multi-tenant adapters (GitHub App without a fixed installation ID, Linear with per-org OAuth), the native client getter requires webhook handler context to resolve credentials. Calling it outside a handler throws. Single-tenant adapters (PAT, API key) work anywhere.
</Callout>
