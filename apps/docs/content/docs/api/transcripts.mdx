...
<Callout type="warn">
  `bot.transcripts` and the `transcripts` config key are deprecated. Use [`bot.history.user`](/docs/api/history#bothhistoryuser) and the `history.user` config key instead — the API surface is identical. `bot.transcripts` will continue to work in the current major version.
</Callout>
