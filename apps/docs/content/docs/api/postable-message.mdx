...
```typescript
type PostableMessage = AdapterPostableMessage | AsyncIterable<string | StreamEvent>;
```
...
An async iterable of strings or stream events. The SDK streams the message in real time using platform-native APIs where available.
