...
<TypeTable
  type={{
    ...
    subject: {
      description: 'Resolves the parent resource (issue, PR, or Notion page) this message is about. Returns null on chat platforms. See [Message Subject](/docs/subject).',
...
<TypeTable
  type={{
    type: {
      description: 'Resource kind, e.g. "issue", "pull_request", or "page".',
      type: 'string',
    },
    id: {
