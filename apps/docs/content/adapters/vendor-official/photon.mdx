...
features:
  ...
  modals:
    status: partial
    label: Remote only, native polls
  slashCommands: no
  mentions:
    status: partial
    label: DMs only
...
<PackageInstall package="@photon-ai/chat-adapter-imessage chat @chat-adapter/state-memory" />
...
<TypeTable
  type={{
    local: {
      type: "boolean",
      description:
        "Run on-device (local) iMessage. Defaults to local unless `local: false`, `IMESSAGE_LOCAL=false`, or remote credentials are provided.",
    },
    projectId: {
      type: "string",
      description:
        "Spectrum Cloud project ID. Falls back to `IMESSAGE_PROJECT_ID`.",
    },
    projectSecret: {
      type: "string",
      description:
        "Spectrum Cloud project secret. Falls back to `IMESSAGE_PROJECT_SECRET`.",
    },
    serverUrl: {
      type: "string",
      description:
        "Self-host gRPC `host:port` (e.g. `imessage.example.com:443`) — not an https URL. Falls back to `IMESSAGE_SERVER_URL`.",
    },
    apiKey: {
      type: "string",
      description:
        "Auth token for a self-hosted server. Falls back to `IMESSAGE_API_KEY`.",
    },
    webhookSecret: {
      type: "string",
      description:
        "Per-webhook signing secret used to verify Spectrum Cloud deliveries. Falls back to `IMESSAGE_WEBHOOK_SECRET`.",
    },
    clients: {
      type: "{ address; token; phone }[]",
      description:
        "Explicit client list for multi-number self-host setups.",
    },
    logger: {
      type: "Logger",
      description: "Chat SDK-compatible logger. Defaults to `ConsoleLogger(\"info\")`.",
    },
  }}
/>
