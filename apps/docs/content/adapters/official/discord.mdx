...
Discord caps a Components v2 message at 40 total components. When a card exceeds that limit the adapter throws a `ValidationError` instead of letting Discord reject the request, so reduce the number of sections, fields, actions, or images on very large cards.
