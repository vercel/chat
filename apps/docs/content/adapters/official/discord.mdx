...
<TypeTable
  type={{
    ...
    componentsV2: {
      type: "boolean",
      description:
        "Render Chat SDK cards with Discord Components instead of embeds. Defaults to `false`.",
    },
    mentionRoleIds: {
      type: "string[]",
      description:
...
Discord cards render as embeds by default. Enable `componentsV2` to render Chat SDK cards with [Discord Components](https://docs.discord.com/developers/components/reference):
