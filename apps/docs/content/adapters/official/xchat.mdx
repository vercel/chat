...
<Callout type="info">
The adapter auto-detects `XCHAT_BOT_TOKEN`, `XCHAT_USER_ID`, `XCHAT_PIN`, and `X_CONSUMER_SECRET` from the environment. The bot's @handle is resolved from `GET /2/users/me` at startup.
</Callout>
...
<TypeTable
  type={{
    ...
    userId: {
      type: "string",
      description:
        "Bot's numeric X user ID. Auto-detected from `XCHAT_USER_ID` or `X_USER_ID`, otherwise resolved from `GET /2/users/me` at startup.",
    },
    pin: {
      type: "string",
      description:
