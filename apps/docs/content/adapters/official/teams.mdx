...
`bot.onInstalled` handles `installationUpdate` actions `add` and `add-upgrade`.
`bot.onUninstalled` handles `remove` and `remove-upgrade`, including personal
installs. Both expose the original `action`, activity `id`, actor `userId`,
`tenantId`, `locale`, platform `conversationId`, and raw activity. These handlers
are currently emitted only by Teams.

A team installation can produce both a bot-join event and an installation event.
Choose one handler for each welcome flow. Filter upgrades when you only want a
first-install welcome; a `remove-upgrade` is not a permanent uninstall. Delivery
may be retried: make side effects idempotent, for example using bot ID, tenant ID,
conversation ID, and activity ID together as an application idempotency key.
There is no automatic subscription, persistence, cleanup, or deduplication.

The optional `conversationReference` is a plain `TeamsConversationReference`,
with bot/user identity, conversation metadata, locale, channel ID, and service
URL. It contains no credentials or live SDK context. Save it in your own durable
storage for later proactive messages; save credentials separately. References
contain personal metadata, so retain only what your application needs.

```typescript
import type { TeamsConversationReference } from "@chat-adapter/teams";
...
bot.onInstalled(async (event) => {
  if (event.adapter.name !== "teams" || !event.conversationReference) return;
  const reference = event.conversationReference as TeamsConversationReference;
  const key = `${event.adapter.botUserId}:${event.tenantId}:${event.conversationId}`;
  // An upsert also replaces the destination after a reinstall or upgrade.
  await installationStore.set(key, JSON.stringify(reference));
...
bot.onUninstalled(async (event) => {
  if (event.adapter.name !== "teams" || event.action !== "remove") return;
  const key = `${event.adapter.botUserId}:${event.tenantId}:${event.conversationId}`;
  await installationStore.delete(key);
});
```

Apply your application's idempotency check before the welcome send. To send
later, load and validate the stored JSON as a `TeamsConversationReference`, then
call `postTeamsMessage` with its `conversation.id` and `serviceUrl`, as above.
This API uses the saved connector endpoint even in a fresh process; do not pass
`activityId` as `replyToId`, since an installation activity is not a message to
reply to. The high-level `bot.channel(id).post()` API does not currently guarantee
that it uses a persisted Teams service URL for proactive sends.

When the activity lacks a service URL, the handler still receives installation
metadata, but `channelId` and `conversationReference` are absent. Cleanup can
still use `conversationId`; a proactive send needs a saved usable reference.
For team installs, this conversation identifies the selected installation
channel, while full team and selected-channel metadata remain in `raw`.
Do not send a farewell after removal. Pass your host's `waitUntil` to the webhook
handler so asynchronous persistence and onboarding finish after the response.
