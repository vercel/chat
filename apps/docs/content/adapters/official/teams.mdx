...
Filter on `action` when you only want a first-install welcome; a `remove-upgrade`
is not a permanent uninstall. Delivery may be retried, so make side effects
idempotent, for example keyed on bot ID, tenant ID, conversation ID, and activity
ID. The SDK does not subscribe, persist, or deduplicate installations for you.
