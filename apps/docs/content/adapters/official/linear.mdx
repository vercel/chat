...
features:
  ...
  streaming:
    status: partial
    label: Agent sessions / Post+Edit
...
<TypeTable
  type={{
    ...
    accessToken: {
      ...
      description:
        "OAuth access token. Accepts a string, or (Vercel Connect) a resolver invoked per API call. Auto-detected from `LINEAR_ACCESS_TOKEN`.",
    },
    clientId: {
      type: "string",
      description:
    ...
    mode: {
      ...
      default: '"comments"',
      description:
        "Set agent-sessions for the recommended Vercel Connect setup with a Linear app-actor installation.",
    },
    webhookSecret: {
      type: "string",
      description:
