...
<CodeBlockTabs defaultValue="react">
  <CodeBlockTabsList>
    ...
    <CodeBlockTabsTrigger value="vue">Vue</CodeBlockTabsTrigger>
    <CodeBlockTabsTrigger value="svelte">Svelte</CodeBlockTabsTrigger>
  </CodeBlockTabsList>
...
<template>
  <!-- chat.messages, chat.sendMessage, chat.status, chat.stop -->
</template>
```
  </CodeBlockTab>
...
The Web adapter speaks the AI SDK UI message stream protocol, so `useChat` from any AI SDK framework package works against the same server endpoint.
...
**Vue / Nuxt** — `@chat-adapter/web/vue` exports the same `useChat` function, returning a `Chat` instance whose `messages`, `status`, and `error` properties are Vue-reactive. Access them directly in your template:
...
<template>
  <div v-for="msg in chat.messages" :key="msg.id">...</div>
</template>
```

**Svelte / SvelteKit** — `@chat-adapter/web/svelte` works identically, returning a `Chat` instance with Svelte 5 `$state`-backed reactive properties:
...
In all three cases the server-side setup is identical — only the import path changes. The `api` and `threadId` options work the same way across all frameworks. Vue and Svelte return the `Chat` instance directly (with `chat.sendMessage()`, `chat.stop()`, `chat.status`, etc.) rather than destructuring, because destructuring Vue or Svelte reactive objects breaks their reactivity tracking.
